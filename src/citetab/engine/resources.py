"""Resolve the paths to citetab's versioned data: rules, profiles, schemas.

The versioned data (``rules/``, ``profiles/``, ``schemas/``) is the single
source of truth under ``src/citetab/_bundled/`` and ships as ordinary package
...
def _resolve(kind: str) -> Traversable:
    ...
    Returns:
        The ``citetab/_bundled/<kind>`` directory as a Traversable.
    ...
    if not bundled.is_dir():
        raise FileNotFoundError(
            f"citetab data directory '{kind}' was not found at '{bundled}'. "
