# TOATool
Tool for updating the Table of Authorities in legal briefs.
toatool
...
brief, you re-run the tool and the table is regenerated correctly — the
way Word's table-of-contents button works, finally applied to
authorities.

What it does
One command. A .docx brief goes in; two files come out:
...
authorities by eyecite
(the Free Law Project's citation parser, tested against tens of
millions of citations).
{brief}.toa-report.md — a readable report: the generated table,
anything you should check before filing, and a disclosure of every
correction the tool made (entries that were missing, page numbers
that were stale, entries it couldn't resolve).
...
for you.
Quick start
bash# Install (requires Python 3.11+ and LibreOffice — see System requirements)
pip install toatool
...
Install LibreOffice with one line:
bash# Debian / Ubuntu
...
Parse the document (python-docx) and extract every citation
in every form — full, short, supra, id., ibid. — with eyecite,
resolving each reference back to its authority.
Build the complete TOA from a versioned court profile (FRAP by
default): grouped, sorted, passim-thresholded, formatted.
Place it deterministically — explicit flag, then [[TOA]]
marker, then the profile's heading variants — and refuse to guess if
none of those match.
Render the document to PDF via LibreOffice headless and locate
every citation on its actual page (pdfplumber).
Iterate to a fixed point — re-render until no citation's page
changes, capped with honest failure disclosure if it won't settle.
Write a copy of the brief with the corrected TOA as static
...
IDNameWhat it disclosesTT-001Unresolved short-form citationA short form with no antecedent full citation; can't be indexedTT-002Missing TOA entryAn authority cited in the body but absent from the input tableTT-003Stale page numbersThe input table's page references describe an older layoutTT-004Phantom TOA entryAn input-table entry that matches nothing cited in the bodyTT-005TOA placement not foundNo place to put the table; output .docx is suppressedTT-006Marker / heading conflictBoth a [[TOA]] marker and a heading exist; marker winsTT-007Non-convergencePagination didn't settle within the iteration capTT-008Font substitutionA render font was substituted; page numbers may shift
...
and held to exactly the same bar. See CONTRIBUTING.md.
License
MIT. See LICENSE. Permissive on purpose: defensibility comes
...
See LIABILITY.md for the full positioning. It is short,
