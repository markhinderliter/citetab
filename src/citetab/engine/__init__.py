...
- :mod:`citetab.engine.resources` resolves the bundled-or-repo paths for the
  versioned ``rules``, ``profiles``, and ``schemas`` data.
- :mod:`citetab.engine.profile_loader` validates a court profile YAML into a
  typed :class:`~citetab.engine.profile_loader.CourtProfile`.
- :mod:`citetab.engine.rule_loader` parses and validates rule-card frontmatter
